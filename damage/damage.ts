...
/**
 * * Cette fonction prend en premier paramètre le personnage à blesser, 
 * * et en deuxième paramètre le total de blessures. 
 * * Sa valeur de retour est le nouveau total de points de vie du personnage :
...
const result = damage({life: 100}, 20);
console.log(result);
...
function domage(eliass: {prenom: string, nom: string, vie: number}, amount: number): number {
    eliass.vie -= amount;
    return eliass.vie;
}
const resultat: number = domage({prenom: 'eliass', nom: 'rachid', vie: 100}, 20);
console.log(resultat);
